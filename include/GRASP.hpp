...
class GRASP : public Metaheuristic
{
    ...
    public:
        ...
        const double alpha, const size_t max_not_improvements, const size_t start_increment)
            : _problemInstance(problemInstance), _engine(seed), _seacher(seacher), _max_time_searching(max_time_searching),
        _start_alpha(alpha), _max_not_improvements(max_not_improvements), _start_increment(start_increment)
        {/* Empty */};
