...
int main(int argc, char const *argv[]) {
    if (argc != 7) {
        ...
        std::cout << "Então os parametros para o algoritmo ILS:" << std::endl;
        std::cout << "Quantidade de perturbações aplicadas na solução" << std::endl;
    ...
    size_t disturbance_rounds = std::strtoul(argv[4], nullptr, 10);
    size_t max_not_improvements = std::strtoul(argv[5], nullptr, 10);

    const long detail_level = std::strtol(argv[6], nullptr, 10);
    ...
    IteratedLocalSearch search(&instance, &neighborhood, &disturbance, disturbance_rounds, max_not_improvements);
